...
{ 
    ...
    private int method5(int M[][],int n) {
        ...
        while (start!=end) {
            // If person end knows start - end is invalid candidate
            if (M[end][start]==1)
                end--;
            
            // Else - start is invalid
            // person start knows end or end does not know start
            else
                start++;
        }
        
        int celeb = start; // Potential candidate for celeb
